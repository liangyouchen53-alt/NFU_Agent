# agent_tools_def.py
"""
工具的 Schema 定義，給 LLM 看的「說明書」。
LLM 會根據每個工具的 description 自己判斷：這個問題該不該呼叫工具、呼叫哪一個、帶什麼參數。
"""
...
TOOLS_SCHEMA = [
    {
        ...
        "function": {
            ...
            "description": (
                "【觸發時機】：當使用者詢問資工系『學分門檻』、『必修/選修科目』、『畢業規定』時呼叫。\n"
                "【RAG 優化關鍵】：傳入的 query 必須是『去背的純關鍵字/科目名』"
                "（例如：將「大一上要修什麼必修」轉為「第一學年 上學期 必修」；"
                "將「畢業要多少學分」轉為「最低畢業總學分」），嚴禁帶入語氣詞或整句對話！"
            ),
            "parameters": {
                "type": "object",
                "properties": {
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                "【觸發時機】：詢問資工系『教授專長』、『研究領域』、『實驗室位置』、『聯絡分機』或職務時呼叫。\n"
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                "【觸發時機】：詢問虎科大『行事曆重要日程』、『放假日期』、『考試週』、『寒暑假開始』時呼叫。\n"
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                "【觸發時機】：當使用者表達『想要前往、查詢、登入』學校的特定平台"
                "（如：數位學習平台、圖書館、校園信箱）或個人校務業務"
                "（如：查課表、看成績、辦理請假、查學雜費）時呼叫。\n"
                "【核心任務】：必須回傳一個包含完整網址的連結，絕對不可以只回覆系統名稱卻不給網址。"
            ),
            "parameters": {
                "type": "object",
                "properties": {
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "查詢羅傑這個虛擬角色的背景設定與黑歷史。僅限閒聊時使用，與校務無關。",
            "parameters": {
                "type": "object",
